...
@Component
@Controller 
public class R2UpdateController {

	//로그객체생성문
	private Logger log=Logger.getLogger(this.getClass());//this.getClass()->현재클래스명
	
	@Autowired
	private R2BoardDao boardDao;//자동으로 Setter Method 호출X=>의존성객체 넣어줌(byType)
	
	//1.글수정 폼으로 이동(GET방식)->반환값(단순 페이지이동=>String)
	//페이지이동 되는데 -> 데이터도 DB연결해서 출력해서 보여줘야한다면=>ModelAndView)
	@RequestMapping(value="/board/R2BoardUpdate.do",method=RequestMethod.GET)
	public ModelAndView form(@RequestParam("board_num") int board_num) { //메서드명은 임의로 작성
		
		System.out.println("다시 처음부터 값을 입력받기 위해서(초기화) form()호출됨");
		System.out.println("board_num=>"+board_num);
		R2BoardVO R2BoardVO=boardDao.selectBoard2(board_num);
		System.out.println("R2BoardVO의 객체"+R2BoardVO);
		//1.이동할 페이지명(확장자생략) 2.키명 3.전달할값
		return new ModelAndView("R2BoardUpdate","command",R2BoardVO);
	}
	
	//2.입력해서 유효성 검사->에러발생
	//BindingResult->유효성검사때문에 필요=>에러정보객체를 저장시키는 역할
	@RequestMapping(value="/board/R2BoardUpdate.do",method=RequestMethod.POST)
	public String submit(@ModelAttribute("command") R2BoardVO com,HttpSession session) {
	    
			if(log.isDebugEnabled()) { //로그객체가 디버깅모드상태인지 아닌지를 체크
			System.out.println("/board/R2BoardUpdate.do 요청중(post");//?을 출력X
			log.debug("R2BoardVO:"+com); //log.debug메서드 사용
			//로그객체명.debug(출력대상자)
		}
		...
		//에러정보가 있다면
		/*
		 * if(result.hasErrors()) { return "R2BoardUpdate"; //그대로 다시 이동해서 입력을 받을 수
		 * 있도록(편집) }
		 */
		//변경전의 데이터를 불러오기 ->board(비밀번호)=웹상에서의 입력비밀번호
		R2BoardVO board=null;
		String oldFileName=""; //변경전 파일명
		board=boardDao.selectBoard2(com.getBoard_num());
	...
	public String process2(@RequestParam("board_num") int board_num) {
		...
		//?,?
		R2BoardVO board=null;
		board=boardDao.selectBoard2(board_num);
